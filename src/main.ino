...
void updateAlarmLogic(float barValue)
{
    if (alarmActive || barValue >= 100.0f)
    {
        if (!alarmActive && barValue >= 100.0f)
        {
            alarmActive = true;
...
void audioTask(void *pvParameters)
{
    while (1)
    {
        ...
        float barValue = normalizeBarValue(averageRMS, activeModeId) * 100.0f;
        updateAlarmLogic(barValue);
        float adjustedRMS = averageRMS - noiseFloor;
        if (adjustedRMS < 0.0f)
            adjustedRMS = 0.0f;

        Serial.printf("Mode: %s (%d), RMS: %.2f, Peak: %ld, DC Offset: %.2f, Avg RMS: %.2f, Observed Max: %.2f, Noise Floor: %.2f, Current Threshold: %.2f, barValue: %.2f \n",
                      modeInfos[activeModeId].name, activeModeId, last_rms, averageRMS, adjustedRMS, observed_max[activeModeId], noiseFloor, currentThreshold, barValue);
